...
from llama_server_tool.__main__ import Greeting, app
...
def test_greet_default() -> None:
    ...
    assert "Hello from llama_server_tool to world!" in result.output
...
def test_greet_named() -> None:
    ...
    assert result.exit_code == 0
    assert "Hello from llama_server_tool to bob!" in result.output
...
def test_greet_formal() -> None:
    ...
    assert result.exit_code == 0
    assert "Good day from llama_server_tool to bob!" in result.output
